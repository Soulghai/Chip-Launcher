...
- (void) loadCostume {
	costume = [CCSprite spriteWithSpriteFrameName:@"chip.png"];
	[costume retain];
...
- (void) setCurrentBodySprite {
    ...
    if (isEyeOpen)
        frame = [[CCSpriteFrameCache sharedSpriteFrameCache] spriteFrameByName:@"chip.png"];
    else
...
- (void) updateLaunch:(ccTime)dt
             _isTouch:(BOOL)_isTouch {
    
    if (_isTouch) {
        velocity = ccpSub(position, costume.position);
    } else {
        if (velocity.x > friction) velocity.x -= friction*dt; else
            if (velocity.x < -friction) velocity.x += friction*dt; else velocity.x = 0;
        
        if (velocity.y > friction) velocity.y -= friction*dt; else
            if (velocity.y < -friction) velocity.y += friction*dt; else velocity.y = 0;
        
        position = ccp(position.x+velocity.x*dt, position.y+velocity.y*dt);
    }
    
    if (position.x < elementRadius) {
        position.x = elementRadius;
    }
    
    if (position.x > 320 - elementRadius) {
        position.x = 320 - elementRadius;
    }
    
    if (position.y < elementRadius) {
        position.y = elementRadius;
    }
    
    [costume setPosition:position];
}
